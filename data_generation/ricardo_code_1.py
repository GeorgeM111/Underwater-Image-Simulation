rfrom matplotlib import pyplot as plt
import numpy as np
import cv2
...
# THERE IS A MAIN IN THE CODE WHICH MEANS THAT THIS CODE CANBE USED INDIVIDUALLY TO RUN AND TEST 
...
gamma = [0.1, 0.1, 0.043] #GM- scattering kernel width per channel R-G-B. different values = wavelength-dependent forward scattering
alpha = [0.032, 0.032, 0.012] #GM - direct-path attenuation per channel RGB, again attenuation is wavelength dependent

# beta = [0.020, 0.005, 0.010]  # beta de ricardo

# A_light = [00.0, 120.0, 30.0]
#GM - particle turbidity params: 15% of pixels get noisy - std dev 1.5 of the guassian blue appriled after the salt noise
turbu_p = [0.15, 1.5]  # noise amount, est. dev.
#GM - particle color per channel RGB
turbu_c = [34.0, 200.0, 201.0]

u = 0.99  # weighting between scattering + attenuation image vs particle noise image

s = 2  # multiplier for particle noise image
depth_add = 0  # addition to values of depht map (a minimum distance for map)
depth_levels = 16 #GM - depth quantization:
#GM - scattering computation divides depth range into 16 bins and for each bin 1 guassian kernel size is computed, all pixels within that depth bin are convolved with that kernel and the contributions are summed up over all bins
kern_size = 10
...
# Compute scatering diffusion for all pixels (Eq. 2, 3) using depth levels
