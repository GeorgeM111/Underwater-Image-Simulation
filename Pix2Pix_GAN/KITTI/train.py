...
"""Pix2Pix GAN / KITTI -- training (clean image_half -> degraded complex image).

Uses the resolution-agnostic ``GeneratorUNet_Dynamic`` so it handles the KITTI
176x608 half-resolution without hard-coded Make3D sizes.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Pix2Pix GAN - KITTI')
    ...
    generator = GeneratorUNet_Dynamic().to(device)
    ...
    writer = SummaryWriter(os.path.join(cfg.runs_dir, 'Pix2Pix_GAN', 'kitti'))

    ckpt_dir = os.path.join(cfg.checkpoint_dir, 'Pix2Pix_GAN')
    ckpt_path = os.path.join(ckpt_dir, 'Pix2Pix_KITTI.ckpt')
    ...
    # PatchGAN output size for the KITTI half-resolution input (H, W from config).
    half_w, half_h = cfg.kitti_half_size
    patch = (1, half_h // 2 ** 4, half_w // 2 ** 4)
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for sample_batched in train_loader:
            ...
            fake_B = generator(input_A)                     # dynamic gen -> same size as input_A
        ...
        writer.flush()
        print('[Pix2Pix KITTI] epoch %d/%d  train_G=%.4f  loss_D=%.4f  val_rel=%.4f' % (
