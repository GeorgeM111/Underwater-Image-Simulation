...
"""Encoder_Decoder_Direct / Make3D -- evaluation script.
...
preserved from the original ``perform_test_1_make_3D.py``; only plumbing/paths/
imports changed to use the shared packages (the intermediate ``.pt`` GT/Pred
dance is folded into a single pass over the test loader).
"""
...
def main():
    parser = argparse.ArgumentParser(description='Encoder_Decoder_Direct Make3D evaluation')
