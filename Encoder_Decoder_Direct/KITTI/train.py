...
"""Encoder_Decoder_Direct / KITTI -- training script.

Direct (encoder->decoder) baseline: a 3-channel image model regresses the complex
(ricardo) degraded image straight from the full-size input. Identical to the Make3D
version except it uses the KITTI loaders; the DenseDepth-style model is resolution
agnostic, so no other change is needed.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Encoder_Decoder_Direct KITTI training')
    ...
    writer = SummaryWriter(os.path.join(cfg.runs_dir, 'Encoder_Decoder_Direct', 'kitti'))
    ckpt_dir = os.path.join(cfg.checkpoint_dir, 'Encoder_Decoder_Direct')
    os.makedirs(ckpt_dir, exist_ok=True)
    ckpt_path = os.path.join(ckpt_dir, 'Models_KITTI.ckpt')
