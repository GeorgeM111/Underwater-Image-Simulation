...
"""Encoder_Decoder_Direct / KITTI -- evaluation script.

Computes accuracy metrics (a1/a2/a3, abs_rel, rmse, log_10) of the direct
encoder->decoder prediction against the complex (ricardo) GT image. Same as the
Make3D version but reads the KITTI test loader.
"""
...
from data.kitti import get_test_loader
from utils.helpers import AverageMeter, DepthNorm, colorize, simple_save_images
from utils.loss import ssim


def main():
    parser = argparse.ArgumentParser(description='Encoder_Decoder_Direct KITTI evaluation')
...
def ClacAccuracyOnly(cfg, args, test_loader):
    ...
    a2_acc = 0.0; cnt_2 = 0
    a3_acc = 0.0; cnt_3 = 0
    abs_rel_acc = 0.0; cnt_4 = 0
    rmse_acc = 0.0; cnt_5 = 0
    log_10_acc = 0.0; cnt_6 = 0

    with torch.no_grad():
        for i, sample_batched in enumerate(test_loader):

            image = sample_batched['image_full'].to(device)
            ...
            if (torch.isfinite(a1)):
                a1_acc = a1_acc + a1.detach().to("cpu").numpy(); cnt_1 = cnt_1 + 1
            if (torch.isfinite(a2)):
                a2_acc = a2_acc + a2.detach().to("cpu").numpy(); cnt_2 = cnt_2 + 1
            if (torch.isfinite(a3)):
                a3_acc = a3_acc + a3.detach().to("cpu").numpy(); cnt_3 = cnt_3 + 1
            if (torch.isfinite(abs_rel)):
                abs_rel_acc = abs_rel_acc + abs_rel.detach().to("cpu").numpy(); cnt_4 = cnt_4 + 1
            if (torch.isfinite(rmse)):
                rmse_acc = rmse_acc + rmse.detach().to("cpu").numpy(); cnt_5 = cnt_5 + 1
            if (torch.isfinite(log_10)):
                log_10_acc = log_10_acc + log_10.detach().to("cpu").numpy(); cnt_6 = cnt_6 + 1
