...
def ClacAccuracyOnly(test_loader):
    ...
    for i in range(need_train+1, N):
        ...
        saving_path_complex_imag_GT = '/sanssauvegarde/homes/t20monda/Test_Resources_2/' + 'Batch_%d' % i + '_Complex_Imag_GT' + '.pt'
        saving_path_complex_imag_Pred = '/sanssauvegarde/homes/t20monda/Test_Resources_2/' + 'Batch_%d' % i + '_Complex_Imag_Pred' + '.pt'
...
def LogProgress(test_loader):
    writer = SummaryWriter('/homes/t20monda/DenseDepth_1/runs1/real_densedepth_weight_2_running')
    model_name = "densenet_multi_task"
    epoch = 0
    checkpoint = torch.load('/sanssauvegarde/homes/t20monda/Dense_Depth_1/checkpoint/' + model_name + '/Models_Weight_2' + '.ckpt')
    ...
    for i, sample_batched in enumerate(test_loader):
        if cnt_batch > need_train : # to skip the last batch from training  
            ...
            # abs_rel, rmse, log_10, a1, a2, a3 = add_results(complex_image_tensor, pred_complex_image, border_crop_size=16)

            # Update step
            total_batch_loss = ((1.0-weight_depth_mean[1]) * loss_complex_total) + (weight_depth_mean[1] * loss_depth)
            ...
            if i == N_Test-1:
                ...
                writer.add_image('Train.6.Ours_Depth_Norm_Normalize_True', colorize(vutils.make_grid(output_depth_norm.data, nrow=6,
                                                                                normalize=True)), epoch)
                
                writer.add_image('Train.7.Ours_Haze_Image_No_Norm', (vutils.make_grid(pred_haze_image.data, nrow=6,
                                                                                normalize=False)), epoch)
                writer.add_image('Train.7.Ours_Haze_Image_No_Norm_Normalize_True', (vutils.make_grid(pred_haze_image.data, nrow=6,
                                                                                normalize=True)), epoch)
