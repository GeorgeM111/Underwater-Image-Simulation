...
def ClacAccuracyOnly(test_loader):
    ...
    for i in range(0, N):
        ...
        saving_path_complex_imag_GT = '/sanssauvegarde/homes/t20monda/DenseDepth_Resources/DenseDepth_1/Simple_Test/' + 'Batch_%d' % i + '_Complex_Imag_GT' + '.pt'
        saving_path_complex_imag_Pred = '/sanssauvegarde/homes/t20monda/DenseDepth_Resources/DenseDepth_1/Simple_Test/' + 'Batch_%d' % i + '_Complex_Imag_Pred' + '.pt'
...
def LogProgress(test_loader):
    writer = SummaryWriter('/homes/t20monda/DenseDepth_1/runs/real_DenseDepth_1_Make_3D_running')
    model_name = "densenet_multi_task"
    epoch = 0
    checkpoint = torch.load('/sanssauvegarde/homes/t20monda/Dense_Depth_1/checkpoint/' + model_name + '/Models_Make_3D' + '.ckpt')
