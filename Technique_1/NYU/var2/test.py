...
"""Technique 1 / NYU / var2 -- evaluation script.
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate Technique 1 NYU var2')
    ...
    # A checkpoint is MANDATORY. Without this, `python test.py` (no --resume) silently
    # evaluated a RANDOMLY-INITIALISED network and printed a plausible-looking table.
    ...
    print('[T%d %s %s] loaded %s (cur_epoch=%s)'
          % (TECHNIQUE, DATASET, VARIANT, ckpt_path, ckpt.get('cur_epoch')))
    ...
    with torch.no_grad():
        for bi, batch in enumerate(test_loader):
            ...
            # var2's models also return their learned loss weights (per-loss + global);
            # only the predictions are needed here (the weights never enter a metric).
            ...
            # max_depth_m must be threaded from cfg: the no-arg fallback reads the module-global
            # CONFIG singleton, so --config would silently not apply to the physics.
            ...
            # Non-finite values are REPORTED, never skipped. The old isfinite-skip left the
            # meter at its default and printed abs_rel/rmse/log10 = 0.0000 for a fully
            # diverged model -- a PERFECT score. Let NaN propagate into the table.
