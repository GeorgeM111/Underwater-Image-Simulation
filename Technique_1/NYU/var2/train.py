...
"""Technique 1 / NYU / var2 -- training script.
...
# on improvement. var2 weights at TWO levels, so both are normalised:
#   0-3  the per-loss L1/SSIM terms under w_depth / w_bb (the depth L1 lives in the
#        reciprocal domain [1, 25] -> O(1-10); its SSIM partner is bounded by 1);
#   4-5  Ld and Lp under the global softmax w_global (loss_depth also carries
#        lambda_grad * gradient_loss, so it is O(1-10) against image losses of O(0.1-0.5)
#        -- which is exactly why w_global drove the depth weight to ~0 and starved the
#        depth head of its only gradient, collapsing the physics to flat airlight).
N_SSIM_D, N_L1_D, N_SSIM_C, N_L1_C, N_DEPTH, N_COMPLEX = 0, 1, 2, 3, 4, 5
N_TERMS = 6


def main():
    parser = argparse.ArgumentParser(description='Train Technique 1 NYU var2')
    ...
    # Technique 1 (paper Eq.13) has NO direct-prediction branch: build_models returns None.
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for batch in train_loader:
            ...
            pred_complex = compute_complex_image(depth_for_complex(out_depth, cfg), out_bb, beta, a_val, unit, image_half,
                                                 max_depth_m=cfg.nyu_max_depth_m)
            w_depth = torch.mean(w_depth, dim=0)
            w_bb = torch.mean(w_bb, dim=0)
            w_global = torch.mean(w_global, dim=0)   # 2 components == the 2 terms of Eq.13 (Ld, Lp)

            ssim_d = ssim_loss(out_depth, depth_n, DEPTH_VAL_RANGE)
            l1_d = l1(out_depth, depth_n)
            grad_d = gradient_loss(out_depth, depth_n)   # DenseDepth edge term; fixed lambda
            ...
            # Technique 1 (paper Eq.13): the global softmax spans Ld and Lp ONLY -- there is NO
            # Lt term (that is T2), which is why _global_terms(1) == 2. The classical haze image
            # is therefore a pure DIAGNOSTIC here (logged + fed to log_health), scored with the
            # FIXED lambdas so its curve stays comparable with 'base'. Built under no_grad so it
            # does not carry a wasted autograd graph.
            #
            ...
            with torch.no_grad():
                ...
                # toward ~1 regardless of quality, and w_global/w_depth/w_bb can shift mass onto
                # whichever term is currently smallest). So the meters are scored with the FIXED
                # cfg lambdas and NO learned weights -- term for term the val loop's v_loss, just
            ...
            total_loss = (w_global[0] * norm(N_DEPTH, loss_depth)
                          + w_global[1] * norm(N_COMPLEX, loss_complex))
            ...
            #
            # w_global is a SOFTMAX (sums to 1): -log(w_i) on every component is the correct
            # barrier, since the only degenerate corners are w_i -> 0. The SIGMOID weights are
            # different -- they enter as a PAIR, (1 - w)*SSIM + w*L1 -- so w -> 1 kills the SSIM
            # term exactly as w -> 0 kills the L1 term, and BOTH endpoints must be barriered.
            # Barriering w alone is not neutral: d/dw[-log w] = -1/w is strictly negative, a
            # monotone push toward w = 1; once the EMA has equalised the two terms the data
            # gradient (L1_n - SSIM_n) is ~0, so that push dominates and pins SSIM at
            # weight_floor -- the barrier would CAUSE the starvation it exists to prevent.
            ...
            total_loss = total_loss + cfg.lambda_weight_reg * weight_log_barrier(
                w_global, w_depth[:1], 1.0 - w_depth[:1], w_bb[:1], 1.0 - w_bb[:1])
        ...
        w_depth_sum, w_bb_sum, w_global_sum, n_val = None, None, None, 0
        with torch.no_grad():
            for batch in val_loader:
                ...
                # simply by shifting softmax mass onto the smallest term. val_avg < best_loss then
                # fires every epoch, early stopping never triggers, and the "best" checkpoint is
                # the MOST collapsed one -- the signal that should expose the collapse certifies
                # it. This is scored exactly as 'base' scores it.
