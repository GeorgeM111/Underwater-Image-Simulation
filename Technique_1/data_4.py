...
ZIP_PATH  = '/datas/sandbox/gmoussa/nyu_data.zip'
BETA_PATH = '/datas/sandbox/gmoussa/parameters/Beta_Mat_NYU_train.npy'
A_PATH    = '/datas/sandbox/gmoussa/parameters/A_Mat_NYU_train.npy'
...
def getTrainingValidationData(batch_size, val_fraction=0.1):
    data, nyu2_train = loadZipToMem(ZIP_PATH)
    beta_mat_arr = load(BETA_PATH)
    a_mat_arr    = load(A_PATH)

    full_dataset = depthDatasetMemory(
        data, nyu2_train, beta_mat_arr, a_mat_arr,
        transform=getDefaultTrainTransform()
    )

    # Deterministic split — same seed every time so val never leaks into train
    n = len(full_dataset)
