...
"""Technique 3 / NYU / var1 -- training script.
...
var1 learns the L1-vs-SSIM balance INSIDE each of the four terms (sigmoid heads);
the four terms themselves are summed with fixed unit weights.
"""
...
# EMA slots — one per sub-loss that is multiplied by a LEARNED weight. Every such term must
# be scale-normalised first: L1 on the reciprocal depth domain [1, 25] is O(1-10) while SSIM
# is in [0, 1], and (1-w)*SSIM + w*L1 is LINEAR in w, so its minimiser is a vertex on
# argmin_i L_i. Un-normalised, the weight is decided by the UNITS of the terms, never by
# which one is actually improving. (See utils/loss_balance.py.)
N_SSIM_D, N_L1_D, N_SSIM_P, N_L1_P, N_SSIM_T, N_L1_T, N_L1_G, N_SSIM_G = range(8)
N_EMA = 8


def main():
    parser = argparse.ArgumentParser(description='Train Technique 3 NYU var1')
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for batch in train_loader:
            ...
            # The OPTIMISED objective: learned weights over EMA scale-normalised sub-losses.
            ...
            total_loss = t_depth + t_complex + t_haze + t_direct
            ...
            total_loss = total_loss + cfg.lambda_weight_reg * weight_log_barrier(w_depth, w_bb, w_dir)
        ...
        w_sums = {'w_depth': None, 'w_bb': None, 'w_dir': None}
        w_count = 0
        with torch.no_grad():
            for batch in val_loader:
                ...
                # by shifting weight onto the smallest term: val_avg < best_loss then fired
                # every epoch, early stopping never triggered, and the "best" checkpoint saved
                # was the maximally collapsed one.
                ...
                for _name, _w in (('w_depth', w_depth), ('w_bb', w_bb), ('w_dir', w_dir)):
        ...
        # they may be read against each other); obj is the actual minimised objective — learned
        # weights over EMA-normalised terms plus the log-barrier. Do NOT compare obj against val:
        # the EMA drives each term to ~1, so obj sits on a scale of its own AND is a moving
