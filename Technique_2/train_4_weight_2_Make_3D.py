...
# we are also using here the "index_haze_image.npz", "index_complex_haze_image.npz" and 
# "index_complex_depth_half_3d.npz" files, during training 
...
def main():
    ...
    # Load data
    train_loader = torch.load('/sanssauvegarde/homes/t20monda/DenseDepth_2/train_loader_make_3D.pkl')
    writer = SummaryWriter('/homes/t20monda/DenseDepth_1/runs/real_DenseDepth_2_Make_3D_weight_1_running')
    ...
    if train_me_where == "from_middle":
        checkpoint = torch.load('/sanssauvegarde/homes/t20monda/DenseDepth_2/checkpoint/' + model_name + '/Models_Weight_2_Make_3D' + '.ckpt')
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            ...
            pred_complex_image = compute_complex_image(output_depth, output_black_box, beta_val_half, a_mat_half, unit_mat_half,
                                                    image_half)
            ...
            total_batch_loss = (weight_features_3_output_mean[0] * loss_complex_total) + (
                    weight_features_3_output_mean[1] * loss_depth) + ((1 - (weight_features_3_output_mean[0] + 
                                                                            weight_features_3_output_mean[1])) * 
                                                                        loss_haze_total)
            running_batch_loss += total_batch_loss.item() * image_full.size(
                0)  # dividing by number of images in the batch
...
def _save_best_model(model_1, model_2, best_loss, epoch):
    ...
    torch.save(state,
               ...
               model_name + '/Models_Weight_2_Make_3D' + '.ckpt')
