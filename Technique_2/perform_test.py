...
from data_3 import getTestingDataOnly
...
def main():
    ...
    batch_size = 10
    # test_loader = getTestingDataOnly(batch_size=batch_size)
    # torch.save(test_loader, '/sanssauvegarde/homes/t20monda/test_loader.pkl')
    
    # Load data
    test_loader = torch.load('/sanssauvegarde/homes/t20monda/DenseDepth_2/train_loader.pkl')
    LogProgress(test_loader)

def LogProgress(test_loader):
    writer = SummaryWriter('/homes/t20monda/DenseDepth_2/runs/real_densedepth_running')
    model_name = "densenet_multi_task"
    epoch = 0
    checkpoint = torch.load('/homes/t20monda/DenseDepth_2/checkpoint/' + model_name + '/Models' + '.ckpt')
    ...
    for i, sample_batched in enumerate(test_loader):
        if cnt_batch > need_train : # to skip the last batch from training  
            ...
            if i == N_Test-1:
                ...
                writer.add_image('Train.6.Ours_Haze_Image_No_Norm', (vutils.make_grid(pred_haze_image.data, nrow=6,
                                                                                normalize=False)), epoch)
                writer.add_image('Train.6.Ours_Haze_Image', colorize(vutils.make_grid(pred_haze_image.data, nrow=6,
                                                                                normalize=False)), epoch)

                writer.add_image('Train.7.Ours_Blackbox_No_Norm', (vutils.make_grid(output_black_box.data, nrow=6, normalize=False)), epoch)
                writer.add_image('Train.8.Ours_Blackbox_Norm', colorize(vutils.make_grid(output_black_box_norm.data, nrow=6,
                                                                                    normalize=False)), epoch)
...
def _save_best_model(model_1, model_2, best_loss, epoch):
    ...
    if not os.path.isdir('/homes/t20monda/DenseDepth_2/checkpoint/' + model_name):
            os.makedirs('/homes/t20monda/DenseDepth_2/checkpoint/' + model_name)

    torch.save(state,
               '/homes/t20monda/DenseDepth_2/checkpoint/' +
