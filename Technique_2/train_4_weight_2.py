...
def main():
    ...
    print("I am in Dense-depth 2 weight 2")
    ...
    # writer_1 = SummaryWriter('runs/real_DenseDepth_2_running')
    ...
    if train_me_where == "from_middle":
        checkpoint = torch.load('/sanssauvegarde/homes/t20monda/DenseDepth_2/checkpoint/' + model_name + '/Models_Weight_2' + '.ckpt')
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            if cnt_batch < need_train:  # to skip the last batch from training    
                ...
                complex_image_tensor = torch.autograd.Variable(
                ...
                output_depth, weight_feature_depth, weight_features_3_output = model_1(image)
                ...
                loss_ssim_complex_image = torch.clamp(
                    (1 - ssim(pred_complex_image.float(), complex_image_tensor.float(),
                              val_range=1)) * 0.5, 0, 1)
                ...
                total_batch_loss = (weight_features_3_output_mean[0] * loss_complex_total) + (
                        weight_features_3_output_mean[1] * loss_depth) + ((1 - (weight_features_3_output_mean[0] + 
                                                                          ...
                                                                          loss_haze_total)
                running_batch_loss += total_batch_loss.item() * image.size(
...
def _save_best_model(model_1, model_2, best_loss, epoch):
    ...
    torch.save(state,
               ...
               model_name + '/Models_Weight_2' + '.ckpt')
