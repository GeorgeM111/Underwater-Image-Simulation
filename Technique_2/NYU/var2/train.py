...
"""Technique 2 / NYU / var2 -- training script.
...
var2 = var1's per-loss sigmoid weights PLUS a global softmax over the three terms
(model_1 -> w_global, 3 components; matches _global_terms(2) in models/model_builder.py).
"""
...
# EMA slots — one per quantity that is multiplied by a LEARNED weight. var2 has TWO levels of
# learned weights, so both levels are normalised: slots 0-5 are the components the per-loss
# sigmoid weights arbitrate over, slots 6-8 the three composed terms the global softmax
# arbitrates over. The gradient term carries a FIXED lambda, so it needs no slot.
N_SSIM_D, N_L1_D, N_SSIM_P, N_L1_P, N_SSIM_T, N_L1_T, N_DEPTH, N_COMPLEX, N_HAZE = range(9)


def main():
    parser = argparse.ArgumentParser(description='Train Technique 2 NYU var2')
    ...
    # Scale-normalise every term BEFORE it meets a learned weight. loss_depth is O(1-10) (an
    # L1 in the reciprocal domain [1, 25] plus lambda_grad * gradient_loss) while the image
    # losses are O(0.1-0.5) on [0, 1] images. A weight minimising sum_i w_i L_i picks
    # argmin_i L_i, so loss_depth was essentially never chosen: w_global[0] -> 0, which
    # starved the depth head of its only gradient and collapsed the physics to flat airlight.
    # Dividing each term by a running estimate of its own magnitude makes the weights
    # arbitrate on IMPROVEMENT rather than on units.
    norm = EMANormalizer(n=9, momentum=cfg.loss_ema_momentum)
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for batch in train_loader:
            ...
            w_global = torch.mean(w_global, dim=0)
            ...
            # OPTIMISED objective: learned weights over EMA-normalised quantities, at both
            # levels (per-loss sigmoid, then global softmax).
            ...
            total_loss = (w_global[0] * norm(N_DEPTH, o_depth)
                          + w_global[1] * norm(N_COMPLEX, o_complex)
                          + w_global[2] * norm(N_HAZE, o_haze))
        ...
        # single dropout-noised sample against epoch-averaged loss curves — and w_global is
        # precisely the curve on which the collapse is visible.
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                unit = batch['unit_mat'].to(device)
                complex_gt = batch['complex_noise_img'].to(device)
                out_depth, w_depth_b, w_global_b = model_1(image_full)
                ...
                # by shifting softmax mass onto the smallest term. val_avg then improves every
                # epoch, early stopping never fires, and the "best" checkpoint saved is the
                # MAXIMALLY COLLAPSED one — the signal that should have exposed the collapse was
                # certifying it. Same fixed lambdas as 'base' -> comparable across variants.
