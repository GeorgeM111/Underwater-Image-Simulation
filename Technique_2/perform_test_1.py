...
def ClacAccuracyOnly(test_loader):
    ...
    for i in range(need_train+1, N):
        ...
        saving_path_complex_imag_GT = '/sanssauvegarde/homes/t20monda/Test_Resources_3/' + 'Batch_%d' % i + '_Complex_Imag_GT' + '.pt'
        saving_path_complex_imag_Pred = '/sanssauvegarde/homes/t20monda/Test_Resources_3/' + 'Batch_%d' % i + '_Complex_Imag_Pred' + '.pt'
...
def LogProgress(test_loader):
    writer = SummaryWriter('/homes/t20monda/DenseDepth_2/runs/real_DenseDepth_2_running')
    model_name = "densenet_multi_task"
    epoch = 0
    checkpoint = torch.load('/sanssauvegarde/homes/t20monda/DenseDepth_2/checkpoint/' + model_name + '/Models' + '.ckpt')
