"""
train_4_var2_make3d.py  –  Technique 4 Variant 2 on Make3D

Extends Variant 1 with a learned global weighting across all four losses.
Includes validation loop, TensorBoard logging, and early stopping.
    ...
    w_global = softmax([w1, w2, w3])  from model_1, sums to 1
    w4 = 1 - (w1+w2+w3)  ≈ 0

    L_total = w1*L_d + w2*L_p + w3*L_t + w4*L_g
"""
...
def main():
    parser = argparse.ArgumentParser(description='Technique 4 Variant 2 – Make3D')
    ...
    ckpt_filename  = "Models_Make3D_Tech4_Var2"
    ...
    writer       = SummaryWriter(os.path.join(BASE_DIR, 'runs', 'Make3D', 'tech4_var2'))
    ...
    for epoch in range(args.epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            ...
            w_depth, w_global, w_residue, w_deg, w_dir = _batch_mean_weights(
                w_depth_sigmoid, w_global, w_residue, w_deg, w_dir)
            ...
            if i % 5 == 0:
                writer.add_scalar('Train/Loss_step',   total_loss.item(),  epoch * len(train_loader) + i)
                writer.add_scalar('Weights/w_depth',   w_global[0].item(), epoch * len(train_loader) + i)
                writer.add_scalar('Weights/w_complex', w_global[1].item(), epoch * len(train_loader) + i)
                writer.add_scalar('Weights/w_haze',    w_global[2].item(), epoch * len(train_loader) + i)
                writer.add_scalar('Weights/w_direct',  w4.item(),          epoch * len(train_loader) + i)
        ...
        with torch.no_grad():
            for sample_batched in val_loader:
                ...
                val_loss, _ = _compute_total_loss(
                    l1_criterion, vgg_loss,
                    output_depth, depth_half,
                    pred_complex, pred_haze,
                    output_direct, complex_image_tensor, orig_haze_image,
...
def _batch_mean_weights(w_depth_sigmoid, w_global, w_residue, w_deg, w_dir):
    return (torch.mean(w_depth_sigmoid, dim=0),
            torch.mean(w_global,        dim=0),
