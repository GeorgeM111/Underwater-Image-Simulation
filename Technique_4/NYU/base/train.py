...
def main():
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for batch in train_loader:
            ...
            pred_complex = compute_complex_image(out_depth.detach(), out_bb, beta, a_val, unit, image_half, max_depth_m=cfg.nyu_max_depth_m)
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                pred_complex = compute_complex_image(out_depth.detach(), out_bb, beta, a_val, unit, image_half, max_depth_m=cfg.nyu_max_depth_m)
                pred_haze = compute_haze_image(out_depth, beta, a_val, unit, image_half, max_depth_m=cfg.nyu_max_depth_m)
                v_loss = lambda_ssim * ssim_loss(out_depth, depth_n, float(depth_n.max())) + lambda_l1 * l1(out_depth, depth_n)
                v_loss = v_loss + cfg.lambda_grad * gradient_loss(out_depth, depth_n)  # DenseDepth edge/gradient term
                v_loss = v_loss + (lambda_l1 * l1(pred_complex, complex_gt) + lambda_ssim * ssim_loss(pred_complex, complex_gt, 1) + lambda_perc * vgg(pred_complex, complex_gt))
                v_loss = v_loss + (lambda_l1 * l1(pred_haze, haze) + lambda_ssim * ssim_loss(pred_haze, haze, 1) + lambda_perc * vgg(pred_haze, haze))
                out_direct = model_3(image_full)
                v_loss = v_loss + (lambda_l1 * l1(out_direct, complex_gt) + lambda_ssim * ssim_loss(out_direct, complex_gt, 1) + lambda_perc * vgg(out_direct, complex_gt))
