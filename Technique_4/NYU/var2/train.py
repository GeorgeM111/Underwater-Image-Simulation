...
"""Technique 4 / NYU / var2 -- training script.
...
Technique 4 total (repo extension of the paper's Eq.21):
    Ltotal = Ld + Lp + Lt + Lg
var2 adds a GLOBAL softmax head on model_1 that weights those four terms. Its width is
_global_terms(4) = 4 in models/model_builder.py, which matches the four terms combined here.
"""
...
# In Technique 4 the per-loss weights sit INSIDE each loss (softmax over L1/SSIM/Perceptual,
# sigmoid over L1/SSIM), so the components -- not just the aggregate losses -- are what those
# weights arbitrate between. Raw, they are not remotely comparable (L1 on [0,1] images ~1e-2,
# the SSIM loss ~1e-1, the VGG term ~1e0, and the depth L1 lives in the reciprocal domain
# [1,25]); since a weight minimising sum_i w_i L_i simply picks argmin_i L_i, the
# largest-magnitude component -- never the worst-fitting one -- was driven to zero weight.
(NX_D_L1, NX_D_SSIM,
 NX_C_L1, NX_C_SSIM, NX_C_PERC,
 NX_H_L1, NX_H_SSIM, NX_H_PERC,
 NX_G_L1, NX_G_SSIM, NX_G_PERC) = range(11)
N_WEIGHTED_TERMS = 11
# Second level: the four aggregate losses meet the GLOBAL softmax, so they are normalised
# too (loss_depth still carries the un-normalised lambda_grad term, so it is O(1) larger
# than the others).
NG_DEPTH, NG_COMPLEX, NG_HAZE, NG_DIRECT = range(4)
N_GLOBAL_TERMS = 4


def main():
    parser = argparse.ArgumentParser(description='Train Technique 4 NYU var2')
    ...
    gnorm = EMANormalizer(n=N_GLOBAL_TERMS, momentum=cfg.loss_ema_momentum)
    ...
    if args.resume:
        ...
        gnorm.load_state_dict(ckpt.get('loss_gnorm'))
    ...
    for epoch in range(start_epoch, cfg.epochs):
        ...
        for batch in train_loader:
            ...
            total_loss = (w_global[0] * gnorm(NG_DEPTH, loss_depth)
                          + w_global[1] * gnorm(NG_COMPLEX, loss_complex)
                          + w_global[2] * gnorm(NG_HAZE, loss_haze)
                          + w_global[3] * gnorm(NG_DIRECT, loss_direct))
            ...
            total_loss = total_loss + cfg.lambda_weight_reg * weight_log_barrier(w_depth_eff, w_residue, w_deg, w_dir, w_global)
            ...
            # total_loss is NOT a reportable loss: the two EMA levels drive each normalised
            # quantity to ~1.0 and both weight levels sum to 1, so it sits near 1.0 whatever the
            # model does. Worse, the EMA scale is a MOVING TARGET, so that curve can fall while
            # the model gets worse, and it is on a different scale from val. What is metered
            # instead is total_fixed: the FIXED cfg lambdas, UNWEIGHTED, no normalisation, no
            # barrier -- term for term the same expression the val loop builds as v_loss, so
            # "train=" and "val=" are the same quantity on two splits. The per-term meters follow
            # the same rule. Reuses the loss tensors already computed above; models not re-run.
            with torch.no_grad():
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # w_global is the curve on which the collapse is visible.
                for _nm, _w in (('w_depth', w_depth), ('w_residue', w_residue),
                                ('w_deg', w_deg), ('w_dir', w_dir), ('w_global', w_global)):
        ...
        state = {'state_dict_1': model_1.state_dict(), 'state_dict_2': model_2.state_dict(),
                 ...
                 'loss_norm': norm.state_dict(), 'loss_gnorm': gnorm.state_dict(),
