...
"""CycleGAN / KITTI -- training. Same as the Make3D version, but the KITTI half
resolution (176x608) is divisible by 4, so the ResNet generator preserves the size
exactly and none of the Make3D F.interpolate size-fixups are needed."""
...
def main():
    ...
    netG_A2B.to(device); netG_B2A.to(device); netD_A.to(device); netD_B.to(device)

    if torch.cuda.device_count() > 1:
        print("Let's use", torch.cuda.device_count(), "GPUs!")
        nn.DataParallel(netG_A2B.cuda()); nn.DataParallel(netG_B2A.cuda())
        nn.DataParallel(netD_A.cuda()); nn.DataParallel(netD_B.cuda())

    netG_A2B.apply(weights_init_normal); netG_B2A.apply(weights_init_normal)
    netD_A.apply(weights_init_normal); netD_B.apply(weights_init_normal)
    ...
    writer_1 = SummaryWriter(os.path.join(cfg.runs_dir, 'CycleGAN', 'kitti'))

    ckpt_path = os.path.join(ckpt_dir, 'CycleGAN_KITTI.ckpt')
    ...
    for epoch in range(start_epoch, n_epochs):
        ...
        for i, sample_batched in enumerate(train_loader):
            num_batches += 1
            input_A = torch.autograd.Variable(sample_batched['image_half'].to(device))
            input_B = torch.autograd.Variable(sample_batched['complex_noise_img'].to(device))

            real_A = input_A.to(device, dtype=torch.float)
            real_B = input_B.to(device, dtype=torch.float)
            ...
            # Identity loss (KITTI size is preserved by the generator -> no interpolate needed)
        ...
        epoch_loss_G = running_batch_losses_G / num_batches
        epoch_loss_D_A = running_batch_losses_D_A / num_batches
        epoch_loss_D_B = running_batch_losses_D_B / num_batches

        lr_scheduler_G.step(); lr_scheduler_D_A.step(); lr_scheduler_D_B.step()

        # ---- validation on held-out split (A->B translation fidelity) ----
        netG_A2B.eval()
        ...
        with torch.no_grad():
            for sample_batched in val_loader:
                ...
                fake_B = from_gan_range(netG_A2B(to_gan_range(real_A)))
        ...
        print('[CycleGAN KITTI] epoch %d/%d  loss_G=%.4f  loss_D_A=%.4f  loss_D_B=%.4f  val_rel=%.4f' % (
